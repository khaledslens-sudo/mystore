...
async function load(){
  try{
    ...
    g.innerHTML=PRODUCTS.map(p=>`<div class=card><div class=no-img>🛍️</div><div class=body><h3>${p.name}</h3><p>${p.description||''}</p><div class=row><span class=price>${p.price} دج</span><button class=add-btn onclick="add('${p.id}')">اضف للسلة</button></div></div></div>`).join('');
