...
async function load(){
  try{
    ...
    g.innerHTML=PRODUCTS.map(p=>`<div class=card><div class=no-img style=${p.image?`background:url('${p.image}') center/contain no-repeat`:''}>${p.image?'':'🛍️'}</div><div class=body><h3>${p.name}</h3><p>${p.description||''}</p><div class=row><span class=price>${p.price} دج</span><button class=add-btn onclick="add('${p.id}')">أضف للسلة</button></div></div></div>`).join('');
